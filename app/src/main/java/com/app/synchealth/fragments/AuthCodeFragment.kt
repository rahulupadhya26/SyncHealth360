 ...
 */
class AuthCodeFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.txtAuthCode.text = Utils.authCode

        binding.btnContinue.setOnClickListener {
            for (i in 0 until mActivity!!.supportFragmentManager.backStackEntryCount) {
                if (mActivity!!.getCurrentFragment() !is LoginFragment) {
                    popBackStack()
                } else {
                    break
                }
    ...
    companion object {
        ...
        const val TAG = "Screen_Auth_code"
    }
}